...
- User will configure CyberArk web app and AgentCore Identity provider
...
Before implementing CyberArk integration, understand the current system architecture and key components.
...
│   ├── council_agent.py         # LLM Council 3-stage deliberation
│   ├── dxo_agent.py             # DxO sequential research workflow
│   ├── bedrock_client.py        # AWS Bedrock client wrapper
│   └── tools/
│       └── arxiv_tool.py        # arXiv API integration
├── backend/                     # FastAPI backend server
│   ├── main.py                  # API endpoints & AgentCore invocation handler
│   ├── config.py                # Configuration (AWS, Bedrock, etc.)
│   ├── models.py                # Pydantic request/response models
│   └── storage.py               # Conversation storage (in-memory)
├── frontend/                    # React + Vite frontend
│   └── src/
│       ├── App.jsx              # Main app component
│       ├── api.js               # API client (axios)
│       └── components/
│           ├── ChatInterface.jsx    # Question input component
│           ├── CouncilView.jsx      # Council results display
│           └── DxOView.jsx          # DxO results display
├── lambda/                      # AWS Lambda proxy (production)
│   └── agentcore_proxy.py       # Lambda handler for AgentCore
└── deploy/                      # Deployment scripts (archived)
```
...
**Important Endpoints**:
```python
# Standard API endpoints (local dev)
POST /api/council          # Run LLM Council deliberation
POST /api/dxo              # Run DxO research workflow
GET  /health               # Health check

# AgentCore Runtime endpoints (production)
GET  /ping                 # AgentCore health check
POST /invocations          # AgentCore invocation endpoint
```

**Key Code Pattern - AgentCore Invocation Handler**:
```python
@app.post("/invocations", response_model=AgentCoreResponse)
async def invocations(request: AgentCoreRequest):
    """
    Receives requests from AWS Bedrock AgentCore Runtime.
    ...
    Routes to appropriate agent and returns formatted markdown response.
    """
...
**Agent Initialization (Line 47-60)**:
```python
@asynccontextmanager
...
**Where to Add CyberArk Endpoint**:
After line 179 (after DxO endpoint), add:
```python
@app.post("/api/cyberark/connect")
async def cyberark_connect():
    """Triggers CyberArk OAuth flow."""
    from agents.cyberark_demo_tool import handle_cyberark_connect
    result = await handle_cyberark_connect()
    return result
```

#### 2. `backend/models.py` - API Data Models

Key models using Pydantic:
```python
class ResearchRequest(BaseModel):
    question: str       # Research question
    mode: str          # "council" or "dxo"

class CouncilResponse(BaseModel):
    question: str
    stage1: List[Dict]  # Independent responses
    stage2: List[Dict]  # Peer rankings
    stage3: Dict        # Chairman synthesis
    metadata: Dict      # Rankings, etc.

class DxOResponse(BaseModel):
    question: str
    workflow: List[Dict]  # 4-step sequential workflow
    metadata: Dict        # Paper count, etc.
```

**For CyberArk, you don't need a new model** - just return a dict directly.
...
No changes needed for CyberArk - OAuth config is in AgentCore.

### Key Agent Components

#### 1. `agents/council_agent.py` - LLM Council

**Architecture**: 3-stage deliberation system
```python
class CouncilAgent:
    async def deliberate(self, question: str) -> Dict[str, Any]:
        # Stage 1: Parallel responses from N members
        stage1 = await self.stage1_collect_responses(question)

        # Stage 2: Anonymized peer review
        stage2, label_map = await self.stage2_collect_rankings(question, stage1)
...
#### 2. `agents/dxo_agent.py` - DxO Research

**Architecture**: 4-step sequential workflow
```python
class DxOAgent:
    async def research(self, question: str) -> Dict[str, Any]:
        # Step 1: Lead Researcher (with arXiv search)
        ...
        # Step 4: Lead Researcher Synthesis
        final = await self.lead_researcher_synthesis(...)

        return {
            'question': question,
            'workflow': [lead_initial, critique, expert, final],
            'metadata': {...}
        }
```
...
#### 3. `agents/bedrock_client.py` - AWS Bedrock Client
...
**For CyberArk**: You'll use a similar pattern but with `@requires_access_token` decorator instead.
...
**State Management**:
```javascript
const [mode, setMode] = useState('council')  // 'council' or 'dxo'
...
const handleSubmit = async (question) => {
    ...
    try {
        let data
        if (mode === 'council') {
            data = await runCouncil(question)  // API call
        } else {
...
**Component Structure** (lines 35-105):
```jsx
<div className="app">
    <header>
...
**Where to Add CyberArk Component**:
After line 41 (after header, before mode selector), insert:
```jsx
{/* Add CyberArk integration */}
<CyberArkConnect />
```

#### 2. `frontend/src/api.js` - API Client
...
export const runCouncil = async (question) => {
    const response = await api.post('/council', { question, mode: 'council' });
    return response.data;
};

export const runDxO = async (question) => {
    const response = await api.post('/dxo', { question, mode: 'dxo' });
    return response.data;
};
```

**For CyberArk**, add this function to `api.js`:
```javascript
export const connectCyberArk = async () => {
    const response = await api.post('/api/cyberark/connect');
    return response.data;
};
```

#### 3. `frontend/src/components/CouncilView.jsx` & `DxOView.jsx`

Display formatted results from agents. Both follow similar patterns:
```javascript
// CouncilView displays 3 stages
<div className="council-results">
    <h2>Stage 1: Independent Responses</h2>
    {result.stage1.map(r => <div>{r.content}</div>)}

    <h2>Stage 2: Peer Rankings</h2>
    {/* Rankings display */}

    <h2>Stage 3: Chairman Synthesis</h2>
    <div>{result.stage3.content}</div>
</div>

// DxOView displays 4-step workflow
<div className="dxo-results">
    {result.workflow.map(step => (
        <div className="step">
            <h3>{step.role} - {step.step}</h3>
            <p>{step.content}</p>
        </div>
    ))}
</div>
...
**CyberArk component will follow similar card-based pattern** but simpler.
...
└───────────────┬─────────────────────────────────────────────┘
                │
                │ 1. User enters question & selects mode
                ▼
┌─────────────────────────────────────────────────────────────┐
│  FRONTEND (React)                                            │
│  - App.jsx: handleSubmit()                                   │
│  - api.js: runCouncil() or runDxO()                         │
└───────────────┬─────────────────────────────────────────────┘
                │
                │ 2. POST /api/council or /api/dxo
                ▼
┌─────────────────────────────────────────────────────────────┐
│  BACKEND (FastAPI)                                           │
│  - main.py: run_council() or run_dxo()                      │
└───────────────┬─────────────────────────────────────────────┘
                │
                │ 3. await agent.deliberate() or agent.research()
                ▼
┌─────────────────────────────────────────────────────────────┐
│  AGENTS                                                      │
│  - council_agent.py: 3-stage parallel + sequential          │
│  - dxo_agent.py: 4-step sequential + arXiv search          │
└───────────────┬─────────────────────────────────────────────┘
                │
                │ 4. Multiple Bedrock API calls
                ▼
┌─────────────────────────────────────────────────────────────┐
│  AWS BEDROCK (Claude 3.5 Haiku)                             │
│  - bedrock_client.py: invoke_async()                        │
└───────────────┬─────────────────────────────────────────────┘
                │
                │ 5. LLM responses
                ▼
...
│  - Council: stage1 → stage2 → stage3                        │
│  - DxO: lead → critic → expert → synthesis                 │
└───────────────┬─────────────────────────────────────────────┘
                │
...
┌─────────────────────────────────────────────────────────────┐
│  FRONTEND DISPLAY                                            │
│  - CouncilView.jsx or DxOView.jsx                           │
│  - Formatted multi-stage results                            │
└─────────────────────────────────────────────────────────────┘
```

### CyberArk Integration Points

Based on the existing architecture, here's where CyberArk will integrate:

```
┌─────────────────────────────────────────────────────────────┐
│  NEW: CyberArk OAuth Flow                                    │
└─────────────────────────────────────────────────────────────┘
                │
                │ 1. User clicks "Connect CyberArk" button
                ▼
┌─────────────────────────────────────────────────────────────┐
│  FRONTEND (NEW)                                              │
│  - CyberArkConnect.jsx: handleConnect()                     │
│  - api.js: connectCyberArk()                                │
└───────────────┬─────────────────────────────────────────────┘
                │
                │ 2. POST /api/cyberark/connect
                ▼
┌─────────────────────────────────────────────────────────────┐
│  BACKEND (NEW ENDPOINT)                                      │
│  - main.py: cyberark_connect()                              │
└───────────────┬─────────────────────────────────────────────┘
                │
                │ 3. await handle_cyberark_connect()
                ▼
┌─────────────────────────────────────────────────────────────┐
│  AGENT TOOL (NEW)                                            │
│  - cyberark_demo_tool.py: @requires_access_token            │
└───────────────┬─────────────────────────────────────────────┘
                │
                │ 4. OAuth flow via AgentCore Identity
                ▼
┌─────────────────────────────────────────────────────────────┐
...
┌─────────────────────────────────────────────────────────────┐
│  FRONTEND DISPLAY                                            │
│  - CyberArkConnect.jsx: show connected state                │
│  - Display user name, email, token expiry                   │
└─────────────────────────────────────────────────────────────┘
```
...
All agent methods are async and return dictionaries:
```python
async def my_agent_method(self, question: str) -> Dict[str, Any]:
...
#### Pattern 3: React Component with API Call
```javascript
...
**Follow these patterns when implementing CyberArk integration** for consistency.
...
No new environment variables needed for CyberArk - OAuth config is in AWS console.
...
No new dependencies needed for CyberArk frontend.
...
**Action Items**:
- [ ] Replace `"CyberArk-Demo"` with actual provider name from AWS console
- [ ] Replace `'INSERT_CALLBACK_URL_HERE'` with callback URL from AWS console
- [ ] Verify scopes match CyberArk application configuration
...
**Action Items**:
- [ ] Add import statement at top of file: `from fastapi import HTTPException`
- [ ] Ensure agents directory is in Python path
- [ ] Test endpoint returns authorization URL on first call
...
**Action Items**:
- [ ] Create both files in `frontend/src/components/`
- [ ] Update backend URL if not using localhost:8001
...
- [ ] Insert component in appropriate location
...
**For Local Development**: Ensure AWS credentials have these permissions in IAM policy.

**Action Items**:
- [ ] Update Lambda execution role in AWS IAM console
- [ ] Verify local AWS profile has required permissions
- [ ] Test permissions by calling `/api/cyberark/connect` endpoint
...
```
pyjwt==2.8.0  # For decoding JWT tokens
```

**Action Items**:
- [ ] Add to requirements.txt
- [ ] Run `pip install -r requirements.txt`
- [ ] If using Lambda, rebuild layer/deployment package
...
# Test endpoint (should return authorization URL on first call)
curl -X POST http://localhost:8001/api/cyberark/connect
```
...
{
  "needs_auth": true,
  "authorization_url": "https://tenant.cyberark.cloud/oauth2/authorize?client_id=...",
  "message": "Please authenticate with CyberArk to continue"
}
```
...
2. Verify CyberArk section appears at top
3. Click "Connect CyberArk" button
4. Popup should open with CyberArk login
5. Authenticate with CyberArk credentials
6. Popup closes automatically
7. UI should update to show:
   - ✓ Connected badge
...
1. Refresh the page
2. Click "Connect CyberArk" again
3. Should immediately show connected (no popup)
4. Verifies token is cached in Token Vault

### Test 4: Error Handling

Test error scenarios:
- Invalid credentials in CyberArk
- Network timeout
